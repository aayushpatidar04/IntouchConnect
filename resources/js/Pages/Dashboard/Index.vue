        ...
        <div class="p-6 space-y-6 animate-fade-in">
			...
			<div>
			    <h3 class="text-sm font-semibold text-surface-700">
			        ...
			        <span
			            v-if="unreadCount > 0"
			            class="ml-1.5 badge bg-red-100 text-red-600 text-[10px]"
			        >
			            {{ unreadCount }}
			        </span>
			    </h3>
...
>
    ...
    <div class="relative shrink-0">
        <div
            class="w-9 h-9 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 text-xs font-semibold"
        >
            {{
                chat.customer_name
                    ?.charAt(0)
        ...
        <!-- Unread indicator -->
        <span
            class="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 flex items-center justify-center bg-red-500 text-white text-[9px] font-semibold rounded-full border-2 border-white"
        >
            {{
    ...
    <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2">
            <p
                class="text-xs font-medium text-surface-800 truncate flex-1"
            >
                {{ chat.customer_name }}
            </p>
        ...
        >
            <p
                class="text-[10px] text-surface-400 font-mono truncate"
            >
                +{{ chat.customer_phone }}
            </p>

            <span
                class="text-[9px] text-surface-400 bg-surface-100 rounded-full px-1.5 py-0.5 truncate max-w-[100px]"
                :title="chat.session_id"
            >
                {{ formatSession(chat.session_id) }}
            </span>
        </div>
    </div>
</div>
...
>
    ...
    <p class="text-[10px] text-surface-300 mt-1">
        No unread messages
    </p>
</div>
                    </div>
